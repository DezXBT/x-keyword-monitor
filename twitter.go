...
// SearchKeyword runs an X search for an arbitrary query via GraphQL SearchTimeline
// and returns the matching tweets, newest first. product="Latest" gives a
// reverse-chronological feed (essential for early detection); "Top" would
// reorder by engagement and bury fresh posts. The raw query supports all X
// search operators (e.g. `"fair launch" -filter:replies min_faves:3`).
func (xc *XClient) SearchKeyword(query string, count int) ([]Tweet, error) {
	if count <= 0 {
		count = 20
	}
	variables := map[string]interface{}{
...
// HealthCheck does a lightweight search request to verify the cookie is alive.
func (xc *XClient) HealthCheck() error {
	_, err := xc.SearchKeyword("the", 1)
	return err
}
